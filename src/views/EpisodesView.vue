<template>
  <div class="card" ref="topRef">
          ...
          <div v-for="(item, index) in slotProps.items" :key="index" class="col-12 md:col-6">
            <div class="card m-3 border-1 surface-border">
              <div class="flex gap-4">
                <img
                  :src="item.image || 'https://rickandmortyapi.com/api/character/avatar/1.jpeg'"
                  :alt="item.name"
                ...
                <div class="w-8">
                  ...
                  <div class="flex flex-1 text-sm mb-1">
                    <span class="font-semibold mr-2 w-4 text-gray-500">Season</span>
                    <span class="font-semibold w-8">: {{ item.episode }}</span>
                  </div>
                  <div class="flex flex-1 text-sm mb-1">
                    <span class="font-semibold text-gray-500 mr-2 w-4">Aired at</span>
                    <span class="font-semibold w-8">: {{ item.air_date }}</span>
                  </div>
                </div>
              </div>
            </div>
    ...
    <CustomizedPagination
      v-if="!loading"
      :totalRecords="result?.episodes?.info?.count"
      :rows="rows"
      :first="first"
...
import GET_ALL_EPISODE from '@/service/EpisodeService'
...
//* Hooks/Methods
const { result, loading, fetchMore } = useQuery(GET_ALL_EPISODE, {
  page: page.value + 1,
  filter: {
    name: keywords.value
  }
